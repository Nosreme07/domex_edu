...
class _AdminUsuariosFormTelaState extends ConsumerState<AdminUsuariosFormTela> with AutomaticKeepAliveClientMixin {
  ...
  void _confirmarExclusaoAcesso(Map<String, dynamic> u) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Deseja realmente marcar ${u['nome']} como EXCLUÍDO?\n\n'
          '• O acesso ao sistema será revogado imediatamente.\n'
          '• O status do cadastro será alterado para "Excluído".\n'
          '• O histórico (como notas e diários) será preservado por segurança.\n\n'
          'Você poderá restaurar o acesso futuramente mudando o status para "Ativo" e gerando uma nova senha.',
        ...
        actions: [
          ...
          ElevatedButton(
                ...
                try {
                  ...
                  if (ctx.mounted) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Usuário marcado como Excluído com sucesso.'), backgroundColor: Colors.green),
                    );
                  }
                } catch (e) {
                  if (ctx.mounted) {
            ...
            child: const Text('Sim, Excluir', style: TextStyle(fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );
  ...
  void _abrirFichaDetalhes(Map<String, dynamic> u, List<Map<String, dynamic>> turmasDoSistema, String codigoEscola) {
        ...
        return StatefulBuilder(
          builder: (context, setStateModal) {
            ...
            if (statusAtualRaw == 'Excluído') {
              statusVisual = 'Excluído';
            } else if (statusAtualRaw != 'Ativo') {
              statusVisual = 'Bloqueado';
            }

            Color corStatusBg = statusVisual == 'Ativo' ? Colors.green.shade50 : (statusVisual == 'Excluído' ? Colors.grey.shade200 : Colors.orange.shade50);
            Color corStatusIcon = statusVisual == 'Ativo' ? Colors.green : (statusVisual == 'Excluído' ? Colors.grey.shade700 : Colors.red);
                        ...
                        children: [
                          SelectableText(
                            ...
                            style: TextStyle(
                              ...
                              decoration: statusVisual == 'Excluído' ? TextDecoration.lineThrough : null,
                              color: statusVisual == 'Excluído' ? Colors.grey : Colors.black87
                            )
                          ),
                  ...
                  child: Column(
                    ...
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(color: corStatusBg, borderRadius: BorderRadius.circular(8), border: Border.all(color: corStatusIcon.withAlpha(50))),
                        child: Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                            Row(
                              children: [
                                Icon(statusVisual == 'Ativo' ? Icons.check_circle : (statusVisual == 'Excluído' ? Icons.highlight_off_rounded : Icons.block), color: corStatusIcon, size: 20),
                                const SizedBox(width: 8),
                                Text('Acesso: ${statusVisual.toUpperCase()}', style: TextStyle(fontWeight: FontWeight.bold, color: corStatusIcon)),
                              ],
                            ),
                            if (statusVisual != 'Excluído')
                              ElevatedButton(
                                style: ElevatedButton.styleFrom(
                                  backgroundColor: statusVisual == 'Ativo' ? Colors.red : Colors.green,
                                  foregroundColor: Colors.white,
                                  elevation: 0,
                                  padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 0),
                                ),
                                onPressed: () async {
                                  final novoStatus = statusVisual == 'Ativo' ? (perfil == 'ALUNO' ? 'Inativo' : 'Bloqueado') : 'Ativo';
                                  try {
                                    if (perfil == 'ALUNO') {
                                ...
                                child: Text(statusVisual == 'Ativo' ? 'Bloquear Acesso' : 'Ativar Acesso', style: const TextStyle(fontSize: 12)),
                              ),
                          ],
                        ),
                      ),
  ...
  void _abrirModalAcessoManual(List alunos, List professores, List responsaveis, List secretaria, Map<String, Map<String, dynamic>> manuaisMap, [Map<String, dynamic>? usuarioEdit]) {
    ...
    List<Map<String, dynamic>> obterLista(String? tipo) {
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (context, setModalState) {
          final corPrimaria = Theme.of(context).primaryColor;
          final listaPessoas = obterLista(tipoSelecionado);
                    ...
                    children: [
                      ...
                      if (isEdicao)
                        ...
                        Autocomplete<Map<String, dynamic>>(
                          ...
                          displayStringForOption: (option) => '${option['nome']} (${getPessoaId(option, tipoSelecionado!)})',
                          optionsBuilder: (TextEditingValue textEditingValue) {
                            ...
                            return listaPessoas.where((p) {
                              final idVal = getPessoaId(p, tipoSelecionado!).toLowerCase();
                              final nome = (p['nome'] ?? '').toString().toLowerCase();
                              return nome.contains(busca) || idVal.contains(busca);
                            });
                          },
                          onSelected: (Map<String, dynamic> val) {
                            setModalState(() {
                              idPessoaSelecionada = getPessoaId(val, tipoSelecionado!);
                                  ...
                                  child: ListView.builder(
                                    ...
                                    itemBuilder: (BuildContext context, int index) {
                                      ...
                                      return ListTile(
                                        ...
                                        subtitle: Text('ID: ${getPessoaId(option, tipoSelecionado!)}'),
                      ...
                      if (!isEdicao && tipoSelecionado == 'ALUNO' && idPessoaSelecionada != null)
                        Padding(
                          ...
                          child: CheckboxListTile(
                            ...
                            onChanged: (val) {
                              setModalState(() {
                                usarMatricula = val ?? false;
                                if (usarMatricula) {
                                  ctrlEmail.text = idPessoaSelecionada ?? '';
                                } else {
                                  ctrlEmail.text = pessoaSelecionada?['email'] ?? '';
                                }
                              });
                            },
                          ),
                      ...
                      TextFormField(
                        ...
                        enabled: !usarMatricula,
                        decoration: InputDecoration(
                          labelText: usarMatricula ? 'Login Gerado' : 'E-mail de Acesso (Login)',
                          hintText: usarMatricula ? '' : 'Digite o e-mail do usuário',
                          border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)),
                          filled: usarMatricula,
                          fillColor: usarMatricula ? Colors.grey.shade100 : Colors.transparent,
                        ),
                        validator: (v) {
              ...
              ElevatedButton.icon(
                ...
                onPressed: salvando ? null : () async {
                  if (formKey.currentState!.validate()) {
                    ...
                    if (!isEdicao) {
                      if (manuaisMap.containsKey(emailFinal) || manuaisMap.values.any((u) => (u['email'] ?? '').toString().toLowerCase() == emailFinal)) {
                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('⚠️ Este login/e-mail já está em uso no sistema!'), backgroundColor: Colors.red));
                        return;
                      }
                    }
                    ...
                    try {
                      ...
                      if (!isEdicao) {
                         ...
                         final idRef = getPessoaId(pessoaSelecionada!, tipoSelecionado!);
  ...
  Widget build(BuildContext context) {
    ...
    void adicionarAuto(Map<String, dynamic> data, String perfil, IconData icone, Color cor) {
      ...
      if (loginChave.endsWith('@domex.com') || loginChave.endsWith('@$codigoEscola.com')) {
         loginChave = loginChave.split('@')[0];
      }
    ...
    final filtrados = todosUsuarios.where((u) {
      ...
      final statusDb = (u['status'] ?? '').toString().toUpperCase();
      String statusAgrupado = 'ATIVO';
      if (statusDb == 'EXCLUÍDO') {
        statusAgrupado = 'EXCLUÍDO';
      } else if (statusDb != 'ATIVO') {
        statusAgrupado = 'BLOQUEADO';
      }

      final matchStatus = _filtroStatus == 'TODOS' || statusAgrupado == _filtroStatus;
                ...
                child: DropdownButtonHideUnderline(
                  child: DropdownButton<String>(
                    ...
                    items: const [
                      ...
                      DropdownMenuItem(value: 'BLOQUEADO', child: Text('Apenas Bloqueados', style: TextStyle(color: Colors.black87))),
                      DropdownMenuItem(value: 'EXCLUÍDO', child: Text('Apenas Excluídos', style: TextStyle(color: Colors.black87))),
                    ],
                  ),
                ),
              ),
            ...
            child: filtrados.isEmpty
                ...
                : ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      Color corBg = Colors.white;
                      Color corBorda = Colors.grey.shade300;
                      
                      if (statusVisual == 'Excluído') {
                        corBg = Colors.grey.shade50;
                        corBorda = Colors.grey.shade300;
                      } else if (statusVisual == 'Bloqueado') {
                      ...
                      return Card(
                        ...
                        shape: RoundedRectangleBorder(
                          ...
                          side: BorderSide(color: corBorda, width: statusVisual == 'Ativo' || statusVisual == 'Excluído' ? 1 : 1.5),
                        ),
                        child: InkWell(
                          borderRadius: BorderRadius.circular(12),
                          onTap: () => _abrirFichaDetalhes(u, turmasDoSistema, codigoEscola),
                          child: Padding(
                            ...
                            child: Row(
                              children: [
                                InkWell(
                                  ...
                                  child: CircleAvatar(
                                    radius: 24,
                                    backgroundColor: u['cor'].withAlpha(30),
                                    backgroundImage: u['fotoUrl'] != null ? NetworkImage(u['fotoUrl']) : null,
                                    child: u['fotoUrl'] == null ? Icon(u['icone'], color: u['cor']) : null,
                                  ),
                                ),
                                const SizedBox(width: 16),
                                Expanded(
                                ...
                                Expanded(
                                  ...
                                  child: Align(
                                    ...
                                    child: Container(
                                      ...
                                      decoration: BoxDecoration(color: u['cor'].withAlpha(20), borderRadius: BorderRadius.circular(6)),
                                      child: Row(
                                        mainAxisSize: MainAxisSize.min,
                                        children: [
                                          Icon(u['icone'], size: 14, color: u['cor']),
                                          const SizedBox(width: 6),
                                          Text(u['perfil'].toString().toUpperCase(), style: TextStyle(color: u['cor'], fontWeight: FontWeight.bold, fontSize: 10)),
                                        ],
                                      ),
                                    ),
                                  ),
                                ...
                                Container(
                                  ...
                                  decoration: BoxDecoration(
                                    color: statusVisual == 'Ativo' ? Colors.green.shade50 : (statusVisual == 'Excluído' ? Colors.grey.shade200 : Colors.red.shade50), 
                                    borderRadius: BorderRadius.circular(8), 
                                    border: Border.all(color: statusVisual == 'Ativo' ? Colors.green.shade200 : (statusVisual == 'Excluído' ? Colors.grey.shade300 : Colors.red.shade200))
                                  ),
                                  child: DropdownButtonHideUnderline(
                                    child: DropdownButton<String>(
                                      value: statusVisual,
                                      icon: Icon(Icons.arrow_drop_down, color: statusVisual == 'Ativo' ? Colors.green.shade700 : (statusVisual == 'Excluído' ? Colors.grey.shade700 : Colors.red.shade700), size: 16),
                                      style: TextStyle(color: statusVisual == 'Ativo' ? Colors.green.shade700 : (statusVisual == 'Excluído' ? Colors.grey.shade700 : Colors.red.shade700), fontWeight: FontWeight.bold, fontSize: 11),
                                      items: const [
                                        DropdownMenuItem(value: 'Ativo', child: Text('ATIVO')),
                                        DropdownMenuItem(value: 'Bloqueado', child: Text('BLOQUEADO')),
                                        DropdownMenuItem(value: 'Excluído', child: Text('EXCLUÍDO')),
                                      ],
                                      onChanged: (novoStatus) async {
                                        if (novoStatus == null || novoStatus == statusVisual) return;

                                        if (novoStatus == 'Excluído') {
                                          _confirmarExclusaoAcesso(u);
                                          return;
                                        }
                                        ...
                                        try {
                                          ...
                                          if (perfil == 'ALUNO') {
                                            ...
                                            await ref.read(usuarioEscolaServiceProvider)!.salvarUsuario({...doc, 'status': statusFinal});
                                          }
                                          
                                          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Status de ${u['nome']} alterado para $novoStatus.'), backgroundColor: Colors.green));
                                        } catch (e) {
