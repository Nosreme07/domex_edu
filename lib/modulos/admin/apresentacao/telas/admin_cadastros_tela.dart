...
class _GestaoAlunosAbaState extends ConsumerState<_GestaoAlunosAba> with AutomaticKeepAliveClientMixin {
  ...
  Future<void> _gerarEImprimirPdf(Map<String, dynamic> aluno) async {
    ...
    if (aluno['fotoUrl'] != null) {
      try { fotoAluno = await networkImage(aluno['fotoUrl']); } catch (e) { debugPrint('Erro ao carregar foto pro PDF: $e'); }
    }

    pw.Widget pdfLinha(String label, dynamic valorRaw) {
      final valor = (valorRaw?.toString() ?? '').trim();
      return pw.Padding(padding: const pw.EdgeInsets.only(bottom: 4), child: pw.Row(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [pw.Text('$label: ', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 9, color: PdfColors.grey800)), pw.Expanded(child: pw.Text(valor.isEmpty ? 'Não informado' : valor, style: const pw.TextStyle(fontSize: 9)))]));
    }
    ...
    pw.Widget pdfBloco(String titulo, pw.Widget conteudo) {
      return pw.Container(
        ...
        child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.stretch, children: [pw.Container(padding: const pw.EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: const pw.BoxDecoration(color: PdfColors.grey200, borderRadius: pw.BorderRadius.vertical(top: pw.Radius.circular(4))), child: pw.Text(titulo, style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10, color: PdfColors.blue800))), pw.Padding(padding: const pw.EdgeInsets.all(8), child: conteudo)])
      );
    }
      ...
      pw.Page(
        ...
        build: (pw.Context context) {
          return pw.Column(
            ...
            children: [
              pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [pw.Text('ESCOLA CONEXÃO', style: pw.TextStyle(fontSize: 22, fontWeight: pw.FontWeight.bold, color: PdfColors.orange700)), pw.Text('Educação que transforma o futuro', style: const pw.TextStyle(fontSize: 10, color: PdfColors.grey700))]), pw.Text('FICHA CADASTRAL DO ALUNO', style: pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold, color: PdfColors.blue800))]),
              pw.SizedBox(height: 12), pw.Divider(thickness: 2, color: PdfColors.orange700), pw.SizedBox(height: 12),
              pw.Expanded(
                child: pw.FittedBox(
                  fit: pw.BoxFit.scaleDown, alignment: pw.Alignment.topLeft,
                  child: pw.Container(
              ...
              pw.SizedBox(height: 4), pw.Divider(thickness: 1, color: PdfColors.grey400), pw.SizedBox(height: 4), pw.Center(child: pw.Text('Ficha cadastral gerada pelo sistema de gestão escolar Domex Edu - $dataHora', style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey600))),
            ]
          );
        }
      )
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_buildSecao(Icons.family_restroom_rounded, 'Responsáveis e Endereço', corPrimaria), if (aluno['responsaveis'] != null) ...(aluno['responsaveis'] as List).map((r) => Padding(padding: const EdgeInsets.only(bottom: 8.0), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_buildLinha('Nome', r['nome']), _buildLinha('CPF', r['cpf']), _buildLinhaTelefone('Tel', r['telefone']), _buildLinha('E-mail', r['email']), const SizedBox(height: 4)]))), const SizedBox(height: 8), _buildLinha('Endereço', '${aluno['endereco']?['rua'] ?? ''}, ${aluno['endereco']?['numero'] ?? ''} - ${aluno['endereco']?['bairro'] ?? ''} - ${aluno['endereco']?['cidade'] ?? ''} / ${aluno['endereco']?['estado'] ?? ''}'), _buildLinha('Referência', aluno['endereco']?['referencia']), _buildLinha('Autorizado sair sozinho?', aluno['autorizaSairSo'] == true ? 'SIM' : 'NÃO')])),
                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_buildSecao(Icons.badge_rounded, 'Autorizados a Buscar', Colors.deepPurple), if (aluno['pessoasAutorizadas'] != null && (aluno['pessoasAutorizadas'] as List).isNotEmpty) ...(aluno['pessoasAutorizadas'] as List).map((p) => _buildLinhaTelefone(p['nome'] ?? 'Autorizado', p['telefone'])) else const Text('Ninguém cadastrado.', style: TextStyle(color: Colors.grey))])),
                      ],
                    ),
                    const Divider(height: 32),
                    Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_buildSecao(Icons.medical_information_rounded, 'Ficha Médica', corPrimaria), _buildLinha('Tipo Sanguíneo', aluno['fichaMedica']?['tipoSanguineo']), _buildLinha('Problema de Saúde', aluno['fichaMedica']?['temProblema'] == true ? 'SIM: ${aluno['fichaMedica']?['problema']}' : 'NÃO'), _buildLinha('Remédio', aluno['fichaMedica']?['tomaRemedio'] == true ? 'SIM: ${aluno['fichaMedica']?['remedio']}' : 'NÃO'), _buildLinha('Alergia', aluno['fichaMedica']?['temAlergia'] == true ? 'SIM: ${aluno['fichaMedica']?['alergia']}' : 'NÃO'), _buildLinha('Observações', aluno['fichaMedica']?['observacoes'])])),
                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_buildSecao(Icons.emergency_rounded, 'Contatos de Emergência', Colors.red.shade400), if (aluno['emergencia'] != null && (aluno['emergencia'] as List).isNotEmpty) ...(aluno['emergencia'] as List).map((e) => _buildLinhaTelefone(e['nome'] ?? 'Emergência', e['telefone'])) else const Text('Nenhum contato cadastrado.', style: TextStyle(color: Colors.grey))])),
                      ],
                    ),
                  ],
                ),
          ...
          actions: [
            TextButton.icon(onPressed: () => _gerarEImprimirPdf(aluno), icon: const Icon(Icons.print_rounded, color: Colors.blue), label: const Text('Exportar PDF / Imprimir', style: TextStyle(color: Colors.blue))),
...
class _GestaoResponsaveisAbaState extends ConsumerState<_GestaoResponsaveisAba> with AutomaticKeepAliveClientMixin {
  ...
  void _confirmarExclusao(BuildContext context, Map<String, dynamic> resp) {
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: const Row(children: [Icon(Icons.warning_amber_rounded, color: Colors.red), SizedBox(width: 8), Text('Excluir Responsável', style: TextStyle(color: Colors.red))]),
  ...
  void _abrirFichaResponsavel(BuildContext context, Map<String, dynamic> resp) {
    final corPrimaria = Theme.of(context).primaryColor;
    final alunosVinculados = resp['alunosVinculados'] as List? ?? [];
      ...
      builder: (context) {
        return AlertDialog(
          ...
          title: Container(
            ...
            child: Row(
              children: [
                InkWell(onTap: resp['fotoUrl'] != null ? () => _mostrarFotoAmpliada(context, resp['fotoUrl']) : null, borderRadius: BorderRadius.circular(32), child: CircleAvatar(radius: 32, backgroundColor: Colors.white, backgroundImage: resp['fotoUrl'] != null ? NetworkImage(resp['fotoUrl']) : null, child: resp['fotoUrl'] == null ? Icon(Icons.family_restroom, size: 32, color: corPrimaria) : null)),
                const SizedBox(width: 16),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(resp['nome'] ?? 'Responsável', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 20)), const SizedBox(height: 4), Text('ID: ${resp['id']}', style: TextStyle(color: Colors.grey.shade600, fontSize: 14))])),
                IconButton(icon: const Icon(Icons.close_rounded), onPressed: () => Navigator.pop(context))
              ],
            ),
          ),
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (alunosVinculados.isEmpty) 
                              const Text('Nenhum aluno vinculado', style: TextStyle(color: Colors.grey))
                            else 
                              ...alunosVinculados.map((a) => Padding(
                                padding: const EdgeInsets.only(bottom: 8.0),
                                child: Row(
                                  children: [
                                    const Icon(Icons.school_rounded, size: 18, color: Colors.blue),
                                    const SizedBox(width: 8),
                                    Expanded(child: Text(a.toString(), style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600))),
                                  ],
                                ),
                              )),
                            const SizedBox(height: 16),
                            const Text('INFORMAÇÕES', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.deepPurple)), 
                            const Divider(), 
                            buildLinha('Status', resp['status'])
                          ]
                        )
                      ),
                    ],
    ...
    try {
      ...
      if (mounted) {
        if (importados > 0 || atualizados > 0) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('$importados novos e $atualizados vínculos de irmãos atualizados!'), backgroundColor: Colors.green));
        } else {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Todos os responsáveis já estavam 100% sincronizados!'), backgroundColor: Colors.blue));
        }
      }
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Erro ao importar: $e'), backgroundColor: Colors.red));
    } finally {
          ...
          Row(
            children: [
              ...
              ElevatedButton.icon(
                ...
                icon: _sincronizando ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)) : const Icon(Icons.sync_rounded), 
                label: const Text('Puxar dos Alunos')
              ),
              const SizedBox(width: 8),
              ElevatedButton.icon(onPressed: () => context.push('/admin/cadastros/responsavel/novo'), icon: const Icon(Icons.person_add_alt_1_rounded), label: const Text('Adicionar Responsável'))
            ],
          ),
          const SizedBox(height: 24),
          Expanded(
            child: estadoResponsaveis.when(
              ...
              data: (lista) {
                final filtrados = lista.where((p) => p['nome'].toString().toLowerCase().contains(_termoBusca.toLowerCase())).toList();
                
                if (filtrados.isEmpty) return const Center(child: Text('Nenhum responsável encontrado. Clique em "Puxar dos Alunos".'));
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      child: Padding(
                        ...
                        child: Row(
                          children: [
                            CircleAvatar(radius: 24, backgroundColor: Colors.grey.shade200, backgroundImage: resp['fotoUrl'] != null ? NetworkImage(resp['fotoUrl']) : null, child: resp['fotoUrl'] == null ? const Icon(Icons.family_restroom, color: Colors.grey) : null),
                            const SizedBox(width: 16),
                            Expanded(
                              flex: 3, 
                              child: Column(
                            ...
                            Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6), decoration: BoxDecoration(color: inativo ? Colors.red.shade50 : Colors.green.shade50, borderRadius: BorderRadius.circular(16)), child: Text(resp['status'] ?? 'Ativo', style: TextStyle(color: inativo ? Colors.red.shade700 : Colors.green.shade700, fontWeight: FontWeight.bold, fontSize: 12))),
                            const SizedBox(width: 24),
                            Row(
                              children: [
                                IconButton(icon: const Icon(Icons.visibility_rounded, color: Colors.blueGrey), tooltip: 'Visualizar Ficha', onPressed: () => _abrirFichaResponsavel(context, resp)),
                                IconButton(icon: const Icon(Icons.edit_rounded, color: Colors.blue), tooltip: 'Editar Cadastro', onPressed: () => context.push('/admin/cadastros/responsavel/novo', extra: resp)),
                                IconButton(icon: const Icon(Icons.delete_rounded, color: Colors.red), tooltip: 'Excluir', onPressed: () => _confirmarExclusao(context, resp)),
                              ],
                            ),
                          ],
                        ),
...
class _GestaoSecretariaAbaState extends ConsumerState<_GestaoSecretariaAba> with AutomaticKeepAliveClientMixin {
  ...
  void _confirmarExclusao(BuildContext context, Map<String, dynamic> membro) {
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: const Row(children: [Icon(Icons.warning_amber_rounded, color: Colors.red), SizedBox(width: 8), Text('Excluir Colaborador', style: TextStyle(color: Colors.red))]),
      ...
      builder: (context) {
        return AlertDialog(
          ...
          title: Container(
            ...
            child: Row(
              children: [
                InkWell(onTap: mem['fotoUrl'] != null ? () => _mostrarFotoAmpliada(context, mem['fotoUrl']) : null, borderRadius: BorderRadius.circular(32), child: CircleAvatar(radius: 32, backgroundColor: Colors.white, backgroundImage: mem['fotoUrl'] != null ? NetworkImage(mem['fotoUrl']) : null, child: mem['fotoUrl'] == null ? Icon(Icons.support_agent_rounded, size: 32, color: corPrimaria) : null)),
                const SizedBox(width: 16),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(mem['nome'] ?? 'Colaborador', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 20)), const SizedBox(height: 4), Text('ID: ${mem['id']}', style: TextStyle(color: Colors.grey.shade600, fontSize: 14))])),
                IconButton(icon: const Icon(Icons.close_rounded), onPressed: () => Navigator.pop(context))
              ],
            ),
          ),
